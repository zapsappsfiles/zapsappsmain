...
const Map: React.FC = () => {
  ...
  const mapOpacity = useTransform(scrollYProgress, [0, 0.5, 1], [0.1, 0.15, 0.1]);
  ...
  return (
    <section ref={sectionRef} className="relative py-24 md:py-32 border-t border-current/10 overflow-hidden">
      {/* NY Map Background */}
      <motion.div 
      ...
      >
        <div 
          className="absolute inset-0 w-full h-full bg-center bg-no-repeat bg-cover filter contrast-50 blur-sm"
          style={{
            backgroundImage: `url('/ny-map-${darkMode ? 'dark' : 'light'}.svg')`,
            opacity: 0.8
          }}
        />
      </motion.div>
